...
const SelectTime: React.FC<Props> = ({
  ...
  return (
    <div
      onClick={onClose}
      className={`select-wrap relative ${widthValue ? ` w-[${widthValue}px]` : `w-full`}`}
    >
      <div
        className={`select bg-white dark:bg-darkInput  ${
          isOpen
            ? `rounded-t-[18px] border-[1px] dark:border-none`
            : `border-[#E6E6E6] border-[1px] rounded-[18px] dark:border-none`
        }`}
      >
        <div className="absolute top-1/2 -translate-y-1/2  md:left-7 left-4 font-montserrat font-normal text-lg leading-6">
          {isOpen && !selectedOption ? startValue : selectedOption}
        </div>
        {isOpen ? (
          <>
            <Image
          ...
          <>
            <Image
              ...
              width={32}
              height={32}
              className="hidden md:block absolute top-1/2 -translate-y-1/2 md:right-7 right-2 ml-auto cursor-pointer dark:invert"
            />
            <Image
              src="/svg/arrow-down.svg"
              alt="Arrov down icon"
              width={24}
              height={24}
              className="md:hidden absolute top-1/2 -translate-y-1/2 md:right-7 right-2 ml-auto cursor-pointer dark:invert"
            />
          </>
        )}
      </div>

      {
        isOpen && (
          <div
            className={`${
              widthValue ? ` w-[${widthValue}px]` : `w-full`
            } absolute overflow-auto h-60 -mt-[2px] bg-white dark:bg-darkInput dark:border-none  rounded-b-[18px] border-[1px] border-[#E6E6E6] border-t-0`}
            ref={selectBodyRef}
          >
            <RSC>
